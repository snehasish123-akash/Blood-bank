...
if (isLoggedIn()) {
    ...
    elseif (isDonor()) redirectTo('donor/dashboard.php');
    else redirectTo('seeker/dashboard.php');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($password) || empty($user_type)) {
        ...
        try {
            ...
            $stmt = $conn->prepare("SELECT * FROM users WHERE (username = ? OR email = ?) AND user_type = ?");
            if (!$stmt) {
            ...
            if ($user) {
                ...
                if ($password === $user['password']) {
                    if ($user['status'] === 'active') {
                        ...
                        redirectTo($user_type . '/dashboard.php');
