    ...
    if (empty($username) || empty($email) || empty($password) || empty($full_name) || empty($user_type)) {
        ...
        try {
            ...
            if ($result->fetch_assoc()) {
                ...
                $stmt = $conn->prepare("INSERT INTO users (username, email, password, user_type, full_name, phone, address, gender, date_of_birth, blood_group, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param(
                    "sssssssssss",
                    $username,
                    $email,
                    $password,      // plain text password here
                    $user_type,
                    $full_name,
                    $phone,
                    $address,
